...
public class BasicIntegrationTest {
    ...
    void testCompleteUserWorkflow() {
        ...
        Page<ProductDto> searchResults = productService.searchProducts("Integration", PageRequest.of(0, 10));
        ...
        ProductDto productDetails = productService.getProductById(testProduct.getId());
        ...
        CartDto clearedCart = cartService.getCart(testUser.getId());
        assertEquals(0, clearedCart.getTotalItems());
        assertEquals(0, clearedCart.getTotalQuantity());

        // Test 11: User can view order
        OrderDto retrievedOrder = orderService.getOrderById(order.getId());
    ...
    void testBusinessLogicValidation() {
        ...
        CartDto updatedCart = cartService.addToCart(testUser.getId(), additionalRequest);
    ...
    void testConcurrentOperations() {
        ...
        CartDto cart1 = cartService.addToCart(testUser.getId(), request1);
        assertEquals(2, cart1.getTotalQuantity());

        UpdateCartItemRequest updateRequest = new UpdateCartItemRequest(testProduct.getId(), 5);
        CartDto cart2 = cartService.updateCartItem(testUser.getId(), updateRequest);
        assertEquals(5, cart2.getTotalQuantity());

        CartDto cart3 = cartService.removeFromCart(testUser.getId(), testProduct.getId());
